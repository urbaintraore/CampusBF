...
import { ShieldAlert, Sparkles, BookOpen, GraduationCap, Building2, FileText, Trophy, Users, Briefcase, Calendar, ShoppingBag, ArrowRight } from 'lucide-react';
import { useNavigate } from 'react-router-dom';

export default function StudentRestrictedSection() {
  const navigate = useNavigate();

  const allowedModules = [
    { icon: Sparkles, name: 'Bourses et Opportunités IA', desc: 'Découvre des opportunités de financement et l\'IA.', to: '/scholarships' },
    { icon: GraduationCap, name: 'Financements', desc: 'Trouve des solutions de financement pour tes études.', to: '/financing' },
    { icon: Building2, name: 'Portail universitaire', desc: 'Accède aux infos et portails de ton université.', to: '/university-portal' },
    { icon: FileText, name: 'Documents', desc: 'Télécharge des cours, TD, TP et examens.', to: '/documents' },
  ...
  return (
    <div className="min-h-[80vh] flex flex-col items-center justify-center py-12 px-4 relative overflow-hidden">
      ...
      <motion.div
        ...
        className="w-full max-w-4xl bg-white/90 backdrop-blur-md rounded-[2.5rem] p-8 sm:p-12 border border-slate-100 shadow-2xl shadow-slate-200/50 relative z-10 text-center"
      >
        ...
        <div className="space-y-4 mb-10 max-w-2xl mx-auto">
          <div className="inline-flex items-center gap-1.5 px-3.5 py-1.5 bg-emerald-50 text-emerald-700 rounded-full text-xs font-bold tracking-wider uppercase">
            ...
            Compte Étudiant CampusBF
          </div>
          <h2 className="text-2xl sm:text-4xl font-display font-bold text-slate-900 tracking-tight">
            Espace non autorisé
          </h2>
          <p className="text-slate-600 text-sm sm:text-base leading-relaxed">
            Tu es au bon endroit ! Mais avec un compte étudiant, tu as uniquement accès aux 10 modules clés ci-dessous. CampusBF est là pour t'accompagner vers la réussite !
          </p>
        </div>

        {/* Bento-like Grid of Allowed Modules */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 text-left mb-10">
